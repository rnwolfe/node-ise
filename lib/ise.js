...
class ISE {
  constructor(host, username, password, bottleNeckConfig = {}) {
    this.host = host;
    this.username = username;
    this.password = password;
    this.isLoggedIn = false;

    // Testing revealed that most calls work great even rapidly/concurrently with the exception of getNodes()
    // for some reason even if only called twice in a row. Limiter may default to no bottlenecking, but leaving
    // here to likely give option to pass in a limiter config on instantiation.

    // Bottleneck configuration
    const limiter = new Bottleneck(
      bottleNeckConfig ? bottleNeckConfig : { minTime: 0 }
    );
    this._get = limiter.wrap(this.__get); // Need to re-consider this naming as this was just for testing.
  }

  async login() {
    // If the user is already logged in, we don't need to do anything.
    // Given the testiness of this API, we'll verify access and return existing object.
    // This is ultimately to save on session creation which can create problems in the long run.
    // This has big impact in long-running processes re-using the same object but ensuring log in.
    if (this.isLoggedIn) {
      if (await this.verifyLogin()) {
        return true; // TODO: Consider returning 'this' here for easier chainability?
      }
    }
    ...
    // As of ISE3.1, the actual login request requires a CSRF token and APPSESSIONID from the login page.
    await this.getPreLoginTokens();
    ...
    // If we have a CSRF token, we'll use it in the login req body. Because ISE 3.1 started using it there.
    if (this.csrfToken) {
      reqBody.OWASP_CSRFTOKEN = this.csrfToken;
    }
    return (
      session
        .post(PATHS.LOGIN, qs.stringify(reqBody), { maxRedirects: 0 })
        .then(response => {
          // Handle errors
          ...
          if (
            ...
            throw this._handleError('Failed to login! Type: detected HTML');
          }
          ...
          if (
            ...
            response.headers.location === '/admin/login.jsp?mid=auth_failed' || // ISE3.0, 3.1
            response.headers.location === '/admin/logout.jsp'
          ) {
            throw this._handleError(
          ...
          if (response.headers['set-cookie']) {
            ...
            return this.getAppSessionId(response);
          }
          return false;
        })
  ...
  getAppSessionId(response) {
    ...
    this.session.defaults.headers.Cookie = sessionIdCookie;
    // eslint-disable-next-line prefer-destructuring
    this.sessionId = sessionIdCookie.split('=')[1];
    return this.sessionId;
  }

  getCsrfToken(include_response = false) {
      ...
      response => {
        ...
        if (response.status === 200) {
          ...
          if (!csrfMatch) {
            ...
            throw this._handleError(
              'Login was successful, but was unable to retrieve the CSRF Token!'
            );
          }
          const csrfToken = csrfMatch[1].split('=')[1];
          this.csrfToken = csrfToken;
          return include_response ? response : this.csrfToken;
        }
        return false;
      }
    );
  ...
  async getPreLoginTokens() {
    const response = await this.getCsrfToken(true);
    await this.getAppSessionId(response);
  }

  /**
   * Logout of ISE and clear existing session.
   *
   * This is best practice as ISE will eventually reach a max session
   * limit and could cause visible impact to admin users.
   *
   * @return {Promise} A promise resolving to true if logout was successful.
   * @memberof ISE
   */
  logout() {
    return this._getWithMeta(PATHS.LOGOUT).then(response => {
      if (
        (this.iseVersion.startsWith('2.') && response.status === 302) ||
        (this.iseVersion.startsWith('3.') &&
          response.status === 200 &&
          response.data.includes('/admin/login.jsp'))
      ) {
        this.isLoggedIn = false;
        this.sessionId = null;
        this.csrfToken = null;
   ...
   * **In ISE UI**: These metrics are displayed on the default Threat dashboard.
   *
   * WARNING: This endpoint often returns a 404 on some systems and can cause an error to throw.
   *
    ...
    return this.session
      ...
      .then(response => {
        if (response.status === 302) {
          if (
            ...
            response.headers.location === '/admin/login.jsp'
          ) {
            throw this._handleError(
              'Redirected to login page, request failed!'
            );
          }
        }

        if (response.status === 404) {
          throw this._handleError('Received HTTP 404', response);
        }

        // Check for errors in response.
        // We shouldn't expect an HTML document here; however, some of the API calls
        // return JSON-like strings that are not valid JSON. So, we have to check for errors in the response.
        // However, if the API session has been logged out, it will instead return a HTML document of the login page.
        if (typeof response.data === 'string') {
          // Check if at login page.
          if (
            response.data.includes('<input name="password" type="hidden"/>')
          ) {
            // Attempt to re-login and attempt the request again using recursion of sorts.
            // return this.login().then(() => this._get(requestPath, config));
            throw this._handleError(
              'Session was logged out. Please login again.'
            );
          }
          // Otherwise, try to fix formatting and JSON.parse.
          try {
      ...
      .catch(error => {
        if (error.code === 'ETIMEDOUT') {
          throw this._handleError(
            'ISE API request timed out. Please try again.',
            error.response
          );
        }
