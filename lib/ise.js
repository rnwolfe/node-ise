...
class ISE {
      ...
      session
        ...
        .then(response => {
          ...
          if (response.headers['set-cookie']) {
            ...
            const sessionCookieArray = response.headers['set-cookie'].filter(
              cookie => cookie.includes('APPSESSIONID')
            );
    ...
    return this._getWithMeta('login.jsp', { maxRedirects: 0 }).then(
      response => {
        ...
        if (response.status === 200) {
          const csrfRegex =
            /<input.*?id="CSRFTokenNameValue".*?value=(.*?) \/>$/m;
  ...
  getApplicationCategoryChartData(filter = 'all') {
    ...
    return this._get(`${PATHS.APPLICATION_CATEGORY_CHART + filter}/all`).then(
