...
class ISE {
  ...
  async createEndpoint(macAddress, data = {}) {
    ...
    if (data.identityGroup) {
      var identityGroup = '';
      throw this._handleError(
        'We can not currently set the identity group statically. Sorry!'
      );
      // This needs to be implemented. Need to resolve name to UUID.
      // var identityGroup = await this.getIdentityGroup({
      //   name: data.identityGroup
      // });
    }
    if (data.policyName) {
      var policyName = '';
      throw this._handleError(
        'We can not currently set the endpoint policy statically. Sorry!'
      );
      // This needs to be implemented. Need to resolve name to UUID.
      // var identityGroup = await this.getEndpointProfilePolicy({
    ...
    const body = {
      ...
      'endPoint.staticEndpoint': policyName ? true : false,
      'endPoint.policyName': policyName || '',
      'endPoint.identityGroup': identityGroup || '',
      'endPoint.staticGroupEndpoint': identityGroup ? true : false,
  ...
  // IDENTITY GROUPS
  getEndpointIdentityGroups(filter, options) {
