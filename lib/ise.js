...
class ISE {
  ...
  getCsrfToken() {
    return this._getWithMeta('', { maxRedirects: 0 }).then(response => {
      ...
      if (response.status === 200) {
        ...
        return { sessionId: this.sessionId, csrfToken: csrfToken };
      }
    });
  }
  ...
  logout() {
    ...
    return this._getWithMeta(LOGOUT, reqConfig).then(response => {
  ...
  visibilityEndpointRequest(filter = {}, options = {}) {
    ...
    return this._getWithMeta(API_PATH + 'visibility', reqConfig);
  }
  ...
  getUsers(filter = {}) {
    return this.visibilityUserRequest(filter, { pageSize: 1 })
  ...
  getApplications(filter = {}) {
    return this.visibilityApplicationsRequest(filter)
  ...
  getNetworkDevices(filter = {}) {
    return this.visibilityNetworkDevicesRequest(filter)
  ...
  getRadiusPolicySets(options) {
    ...
    const query = { ...defaults, ...options };
    return this._get(RADIUS_POLICY_SETS, this._addQPH(query));
  }
  ...
  getRadiusSecurityGroups() {
    return this._get(RADIUS_SECURITY_GROUPS);
  }

  getRadiusServiceNames() {
    return this._get(RADIUS_SERVICE_NAMES);
  }

  getRadiusAuthorizationProfiles() {
    return this._get(RADIUS_AUTHZ_PROFILES);
  }

  getRadiusIdentityStores() {
    return this._get(RADIUS_IDENTITY_STORES);
  }

  getRadiusFailovers() {
    return this._get(RADIUS_FAILOVERS);
  }

  getRadiusAuthenticationPolicy(policySet) {
    return this._get(RADIUS_AUTHENTICATION_POLICY(policySet));
  }

  getRadiusAuthorizationPolicy(policySet) {
    return this._get(RADIUS_AUTHORIZATION_POLICY(policySet));
  }

  getRadiusLocalExceptions(policySet) {
    return this._get(RADIUS_LOCAL_EXCEPTIONS(policySet));
  }

  getRadiusGlobalExceptions() {
    return this._get(RADIUS_GLOBAL_EXCEPTIONS).then(response => {
  ...
  getTacacsPolicySets(options) {
    const defaults = { startAt: 1, pageSize: 25 };
    const query = { ...defaults, ...options };
    return this._get(TACACS_POLICY_SETS, this._addQPH(query));
  }
  ...
  getTacacsServiceNames() {
    return this._get(TACACS_SERVICE_NAMES);
  }

  getTacacsShellProfiles() {
    return this._get(TACACS_SHELL_PROFILES);
  }

  getTacacsCommandSets() {
    return this._get(TACACS_COMMAND_SETS);
  }

  getTacacsIdentityStores() {
    return this._get(TACACS_IDENTITY_STORES);
  }

  getTacacsFailovers() {
    return this._get(TACACS_FAILOVERS);
  }

  getTacacsAuthenticationPolicy(policySet) {
    return this._get(TACACS_AUTHENTICATION_POLICY(policySet));
  }

  getTacacsAuthorizationPolicy(policySet) {
    return this._get(TACACS_AUTHORIZATION_POLICY(policySet));
  }

  getTacacsLocalExceptions(policySet) {
    return this._get(TACACS_LOCAL_EXCEPTIONS(policySet));
  }

  getTacacsGlobalExceptions() {
    return this._get(TACACS_GLOBAL_EXCEPTIONS).then(response => {
      if (response === '' || !response) {
        return undefined;
      }
      return response;
  ...
  getApplicationCategoryChartData(filter = 'all') {
    ...
    return this._get(APPLICATION_CATEGORY_CHART + filter + '/all').then(data =>
  ...
  getGenericDashboard(query = 'alarms') {
    return this._get(GENERIC_DASHBOARD, this._addQPH({ query: query }));
  }
  ...
  getVulnerableEndpointsOverTime(period = 'day') {
    ...
    return this._get(`${VULNERABILITY_ENDPOINTS_OVER_TIME}/${period}/0`);
  }
  ...
  getRadiusLiveLogs(filter, options) {
    ...
    const reqConfig = this._addQPH(query, filter);
    return this._get(RADIUS_LIVE_LOGS, reqConfig);
  }

  getRadiusLiveLogCounters() {
    return this._get(RADIUS_LIVE_LOG_COUNTERS);
  }
  ...
  getTacacsLiveLogs(filter, options) {
    ...
    const reqConfig = this._addQPH(query, filter);
    return this._get(TACACS_LIVE_LOGS, reqConfig);
  }

  getTcNacLiveLogs(filter, options) {
    return this._get(TCNAC_LIVE_LOGS);
  }
