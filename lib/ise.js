...
class ISE {
    ...
    return session
      ...
      .then((response) => {
        ...
        if (response.data.includes('<html')) {
          this._handleError('Failed to login!', response);
        }

        // No cookies means something went wrong.
        // Even bad credentials give an APPSESSIONID that doesn't work.
        if (!response.headers['set-cookie']) {
          this._handleError('Response did not contain any cookies!', response);
        }
        ...
        if (response.headers['set-cookie']) {
          ...
          if (!('appsessionid' in cookies)) { this._handleError('Missing APPSESSIONID cookie. An error occurred in authentication. Double check credentials.', response); }
          // Successful login should have an APPSESSIONID and TOKEN cookie.
          const sessionCookieArray = response.headers['set-cookie'].filter((cookie) => cookie.includes('APPSESSIONID'));
      ...
      .then(() => this.getNodes().then((nodes) => {
        ...
        this._handleError(`${'Login failed, please check your ISE hostname and credentials. '
          ...
          + 'This library has only been *successfully* tested against ISE versions: '}${TESTED_VERSIONS.join(', ')}`);
      }));
  }
  ...
  resolveAllEndpoints(response, filter = {}) {
    ...
    // pagination
    const [startItem, endItem, totalEndpoints] = response.headers['content-range']
      .split(/[-/]/)
      .map((e) => parseInt(e));
    ...
    if (hasNextPage) {
      promises.push(
        this.visibilityEndpointRequest(filter, { startAt: pageNum + 1, pageSize }).then(
          (endpoints) => this.resolveAllEndpoints(endpoints),
        ),
      );
    }
  ...
  resolveAllUsers(response, filter = {}) {
    ...
    if (hasNextPage) {
      promises.push(
        this.visibilityUserRequest(filter, { startAt: pageNum + 1, pageSize }).then(
          (users) => this.resolveAllUsers(users),
        ),
      );
    }
    return Promise.all(promises);
  ...
  getApplicationCategoryChartData(filter = 'all') {
    ...
    return this._get(`${PATHS.APPLICATION_CATEGORY_CHART + filter}/all`).then((data) => data.map((item) => JSON.parse(item)));
  }

  // DASHBOARDS
  getGenericDashboard(query = 'alarms') {
    return this._get(PATHS.GENERIC_DASHBOARD, this._addQPH({ query }));
  }
  ...
  autoGenPxGridCert(commonName, description, password, format = 'pkcs8', certpath = null) {
    ...
    return this._post(`${PATHS.API}pxGrid/certificate/create`, reqBody, reqConfig)
      ...
      .then((filepath) => unzip(filepath, { dir: fileDir }).then((err) => {
        if (err) {
          throw new Error(err);
        }
        return certConfig;
  ...
  _createIseSession() {
    return http.create({
      ...
      validateStatus:
        (status) => ((status >= 200 && status <= 299) || status === 404 || status === 302),
    });
  }
